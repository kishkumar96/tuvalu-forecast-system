...
def create_table():
    ...
    df['Vis'] = df['Vis'].fillna(0).replace([np.inf, -np.inf], 0).astype('int')
    df['PkDir'] = df['PkDir'].fillna(0).replace([np.inf, -np.inf], 0).astype('int')
    df['Mwd'] = df['Mwd'].fillna(0).replace([np.inf, -np.inf], 0).astype('int')
    df['WD'] = df['WD'].fillna(0).replace([np.inf, -np.inf], 0).astype('int')
