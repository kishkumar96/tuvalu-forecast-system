...
def hallsTailoredForecast(now, configfile):
    ...
    for index, row in df.iterrows():
        ...
        ##SCP everything
        #result = subprocess.run("scp -r /home/divesha/data/TUV/tailored-report-tv/Hall_Reports/"+ now.strftime("%Y") + now.strftime("%m") + now.strftime("%d") +  now.strftime("%H")+" divesha@opmdata.gem.spc.int:/var/www/html/file",shell=True, capture_output=True, text=True)
    ...
    try:
        ...
        print(f'⚠️  Email sending failed (skipped): {e}')
        print('✅ Reports generated successfully in output directory')
