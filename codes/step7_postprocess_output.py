...
def postprocess_SWAN(now, plot):
    ...
    if not os.path.exists(fl_name):
        raise FileNotFoundError(f"Missing SWAN output file: {fl_name}")
    ...
    # The ADCIRC mesh file (fort.14) uses 1-based indexing for nodes, but
    # matplotlib requires 0-based indices (from 0 to N-1). The error indicates
    # an out-of-bounds index, which suggests an issue in the source fort.14 file.
    # The correct conversion is to subtract 1 from the node indices.
    triangles_0based = mesh.triangles - 1
    triang = tri.Triangulation(x, y, triangles_0based)
